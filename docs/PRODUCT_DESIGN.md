...
只有当前正在编辑的工程展开 Story、Character、Scene、Shot、Timeline、版本入口与检查结果，其他工程
保持单行折叠状态，点击即可切换。右侧使用更大的可视区域展示媒体预览、Take、Shot、Current Cut 和
Revision。计划、运行进度、审批和错误都作为对话内容出现，不另设底部任务控制台。依赖图、日志、
参数等高级信息按需展开。

界面采用渐进披露：默认输入区只保留 Harness 和发送动作，普通消息直接返回对话文本。图片、视频
等工具类型和媒体提供方在“生成设置”中一次性选择，发送后恢复为仅对话；工具进度只在执行期间
临时出现。空工程的资源区不展示无意义的搜索框和空列表，而是给出“从对话创建第一个场景”
的单一入口；组装、渲染等动作只有在存在所需镜头或已选 Take 时才可用。每个已打开工程拥有独立
Core 会话，任务运行时仍可切换到其他工程并继续工作；失败任务保留原目标并提供“修改后重试”，
避免上下文丢失和无效操作。
...
1. 用户在版本历史中选择一个旧 Revision。
2. OpenMovie 展示该版本的时间、作者和修改摘要。
3. 用户点击“恢复为新版本”。
4. OpenMovie 把旧快照追加为新的 Revision，原历史不受影响。
5. 用户继续对新 Revision 进行对话式修改。
