...
- 左侧 Project Explorer 从应用级 Recent Projects 索引读取全部工程，只展开当前选中工程的 Story、Character、Scene、Shot、Timeline 与版本摘要。Desktop Main 为每个已打开工程保留独立 Core 子进程；切换时只改变活动会话并重新获取该工程投影，不关闭其他工程、不取消其后台任务，也不改变侧栏顺序。
